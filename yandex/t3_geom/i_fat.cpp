...
reach check(ll from, ll to, fl l) {
    adj.assign(N + 4, {});
    cmp.assign(N + 4, -1);

    const ll v_left = N, v_right = N + 1, v_up = N + 2, v_down = N + 3;
    
    for (ll i = 0; i < N; i++) {
        for (ll j = i + 1; j < N; j++) {
            if (pts[i].to(pts[j]).len() - fl(rds[i]) - fl(rds[j]) <= l) {
                adj[i].push_back(j);
                adj[j].push_back(i);
            }
        }
    ...
    for (ll i = 0; i < N; i++) {
        if (pts[i].x + fl(rds[i]) + l >= fl(w)) {
            adj[i].push_back(v_left);
            adj[v_left].push_back(i);
        }

        if (pts[i].x - fl(rds[i]) - l <= 0) {
            adj[i].push_back(v_right);
            adj[v_right].push_back(i);
        }

        if (pts[i].y + fl(rds[i]) + l >= fl(h)) {
            adj[i].push_back(v_up);
            adj[v_up].push_back(i);
        }

        if (pts[i].y - fl(rds[i]) - l <= 0) {
            adj[i].push_back(v_down);
            adj[v_down].push_back(i);
        }
    }
