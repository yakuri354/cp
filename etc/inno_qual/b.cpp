...
struct bigint {
    ...
    friend bigint gcdb(const bigint &a, const bigint &b) { return b.isZero() ? a : gcdb(b, a % b); }

    friend bigint lcmb(const bigint &a, const bigint &b) { return a / gcdb(a, b) * b; }
...
struct rr {
    ...
    rr(bigint p, bigint q): p(std::move(p)), q(std::move(q)) {
    }

    rr(bigint x): p(std::move(x)), q(1) {}

    rr operator+(const rr &o) const {
        rr x = {p * o.q + o.p * q, q * o.q};
        return x.r();
    }
...
void solve() {
    ...
    rr num = rr(A).r() + rr(B, pw10[m]).r() + rr(C, (pw10[k] - bigint(1)) * pw10[m]).r();

    bigint q = num.q;

    vector<bigint> ans;
    for (bigint i = 2; i * i <= q; i = i + bigint(1)) {
        if (q % i == 0) {
            ans.push_back(i);
        }
        while (q % i == 0) {
    ...
    bigint ansv = 1;
    for (const bigint &i: ans) {
        ansv = (ansv * i) % mod;
    }

    cout << ansv.longValue() << endl;
}
